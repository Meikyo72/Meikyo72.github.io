  ...
  class RealObject extends React.Component {
    render() {
        let newodatalist = oDataList.map(
          item => <div key={item.objectKey} className="col-lg-3 col-md-2 col-sm-4 col-xs-6 filtr-item filterFilter" data-category={item.flat ? '1' : item.house ? '2' : item.building ? '3' : item.locale ? '3' : '9'}
